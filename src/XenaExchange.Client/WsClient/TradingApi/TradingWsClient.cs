...
namespace XenaExchange.Client.WsClient.TradingApi
{
    /// <summary>
    ...
    public class TradingWsClient : WsClientBase, ITradingWsClient
    {
        ...
        public TradingWsClient(TradingWsClientOptions options, ISerializer serializer, ILogger<TradingWsClient> logger)
        ...
        {
            Validator.GrThanOrEq(nameof(accountId), accountId, (ulong)0);
            Validator.NotNullOrEmpty(nameof(symbol), symbol);
            if (requestId != null)
                Validator.NotNullOrEmpty(nameof(requestId), requestId);

            var request = new PositionMaintenanceRequest
            {
                MsgType = MsgTypes.PositionMaintenanceRequest,
                Account = accountId,
                Symbol = symbol,
                PosReqID = requestId,
                PosTransType = PosTransType.Collapse,
        ...
        public async Task CancelOrderByClOrdIdAsync(string clOrdId, string origClOrdId, string symbol, string side, ulong account)
        {
            ...
            Validator.GrThanOrEq(nameof(account), account, (ulong)0);
